...
export async function GET(ctx: APIContext) {
  ...
  const manifest = await manifestObj.json<{ version: string; macX64File?: string }>();
  const fileName = manifest.macX64File;

  if (!fileName) {
    return new Response(JSON.stringify({ error: 'macOS Intel build not available yet' }), {
      status: 404,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  ctx.locals.runtime.ctx.waitUntil((async () => {
    try {
      ...
      const [totalRaw, dailyRaw, macRaw, geoRaw] = await Promise.all(reads);
