...
- ✅ **N-6 — FIXED.** `extend-trial` valida `machineId` contra el formato UUID (lo que manda la app: `crypto.randomUUID()`) → un valor crafteado (ej. un email) no puede colisionar con la namespace `trial:<email>` ni inflar keys.
- ✅ **N-7 — FIXED.** `heartbeat` valida `tunnelUrl` (allowlist: https + `*.trycloudflare.com` / `*.patapim.ai`); cualquier otra cosa se guarda como `null`. Cierra el SSRF oracle de `device/list` + `device/debug` (`fetch(tunnelUrl + '/ping')`).
