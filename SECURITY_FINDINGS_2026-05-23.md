...
- ⏳ **BE-5 `referral/status`** — pendiente. Requiere plomería de auth en el **renderer** (`referralPanel.js` llama con `{email}` sin token). Fix correcto: rutear por main/IPC con device token y scopear al usuario autenticado. Más invasivo (cross-repo).
- ⏳ **BE-6 `extend-trial`** — pendiente / a investigar. **Sin caller** detectado en ninguno de los dos repos; exigir auth podría romper un flujo no visible. Bajo impacto (trials). Confirmar quién lo llama antes de tocar.
- ⏳ **BE-7 Telegram WS** (`instance/[id]` + DO `handleUpgrade`) — pendiente. Auth/ownership a nivel Durable Object; diseño aparte.
- ⏳ **BE-8 `referral/invite` + `referral/claim`** — pendiente. Mismo patrón que BE-5 (callers en renderer/main sin auth). Fraude/spam de referidos, valor menor.
